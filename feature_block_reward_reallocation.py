...
class BlockRewardReallocationTest(DashTestFramework):
    ...
    def signal(self, num_blocks, expected_lockin):
        ...
        for i in range(500 - num_blocks):
            ...
            self.log.info("Created test block")
            self.nodes[0].submitblock(ToHex(test_block))
            self.log.info("Submitted test block")
        ...
        if num_blocks > 0:
            ...
            assert_equal((self.nodes[0], 'v17')['status'], 'started')
    ...
    def run_test(self):
        self.log.info("Wait for Initial state STARTED")
        while get_bip9_status(self.nodes[0], 'v17')['status'] != 'started':
        ...
        self.log.info("Checking for state LOCKED IN")
        bi = self.nodes[0].getblockchaininfo()
        assert_equal(bi['blocks'], 498)
        assert_equal(bi['bip9_softforks']['v17']['status'], 'locked_in')

        self.log.info("Advance from LOCKED IN to ACTIVE at height = 499")
        self.nodes[0].generate(1)
        bi = self.nodes[0].getblockchaininfo()
        assert_equal(bi['blocks'], 499)
        assert_equal(bi['bip9_softforks']['v17']['status'], 'active')
        #assert_equal(bi['bip9_softforks']['realloc']['statistics']['threshold'], self.threshold(0)) # not applicable
        #sleep(1000)
